...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.environ.get('DATABASE_URL', 'postgresql://postgres:postgres@localhost:5432/warbler'))
...
def do_logout():
    """Logout user."""
    user_key = session[CURR_USER_KEY]
    if CURR_USER_KEY in session:
...
def logout():
    ...
    # do_logout now returns user id along with log out the user
    id = do_logout()
    user = User.query.filter_by(id = id).first()
    #we do a try catch here because of the off chance the user isnt found
    #we wont crash the application and instead handle it with a generic message
    try:
        flash(f"Till next time {user.username}!", "success")
    except:
